...
class MujocoARConnector:
    ...
    def __init__(self, mujoco_model=None, mujoco_data=None, port=8888, debug=False, hand_transformation=None):
        """
        ...
        self.port = port
        self.hand_transformation = np.array(hand_transformation) if hand_transformation is not None else None
        self.latest_data = {
            "rotation": None,
            "position": None,
            "finger_angles": None,
            ...
            async for message in websocket:
                if self.get_updates:
                    ...
                    if 'rotation' in data and 'position' in data:
                        ...
                        if self.hand_transformation is not None:
                            T_phone_hand = self.hand_transformation
                            
                            T_world_phone = np.eye(4)
                            T_world_phone[:3, :3] = self.latest_data["rotation"].reshape(3, 3)
                            T_world_phone[:3, 3] = self.latest_data["position"]
                            
                            T_world_hand = T_world_phone @ np.linalg.inv(T_phone_hand)
